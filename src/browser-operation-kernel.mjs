...
if (!__twLocator) throw new Error("TOOLWIRE_BROWSER_SCOPE_TARGET_COUNT:-1:0");
const __twFillStrategy = "fill"; const __twCount = await __twLocator.count(); /* if (false) { const __twVisibleRoleTargets = []; } */`;
  }
...
function envelope(kind, body, { keep = false, claim = true, preDispatchRelease = false, preserve = [] } = {}) {
  ...
  return `
let __twTab = null; let __twPayload = null; let __twDispatchAttempted = false; let __twActionError = null; let __twFinalizeError = null;
...
if (__twActionError) throw __twActionError; if (__twFinalizeError) throw __twFinalizeError; nodeRepl.write(JSON.stringify(__twPayload));`;
}
...
function sourceFor(operation, input) {
  ...
  switch (operation) {
    ...
    case OPERATIONS.scroll: {
      ...
      const body = `const __twBeforeUrl = (await __twTab.url()) ?? __twInfo.url ?? null; if (__twBeforeUrl !== (${quote(input.expectedUrl)})) throw new Error("TOOLWIRE_BROWSER_ACTION_URL_CHANGED"); __twDispatchAttempted = true; const __twBody = __twTab.playwright.locator("body"); for (const __twKey of ${quote(keys)}) await __twBody.press(__twKey, { timeoutMs: 3000 }); const __twScrollReturned = true; __twPayload = { beforeUrl: __twBeforeUrl, scrollReturned: true, inputMethod: "body-keypress", keypresses: ${quote(keys)}, settleCompleted: false }; await __twTab.playwright.waitForTimeout(350); __twPayload.settleCompleted = true;`;
      return `${header}\nconst __twInfo = (await __twBrowser.user.openTabs()).find((tab) => tab.providerTabId === ${quote(input.providerTabId)}); if (!__twInfo) throw new Error("TOOLWIRE_BROWSER_TAB_STALE"); ${envelope("scroll", body)}`;
    }
    case OPERATIONS.keypress: {
      const body = `const __twBeforeUrl = (await __twTab.url()) ?? __twInfo.url ?? null; if (__twBeforeUrl !== (${quote(input.expectedUrl)})) throw new Error("TOOLWIRE_BROWSER_ACTION_URL_CHANGED"); __twDispatchAttempted = true; await __twTab.dom_cua.keypress({ keys: [${quote(input.key)}] }); __twPayload = { beforeUrl: __twBeforeUrl, afterUrl: await __twTab.url() ?? null, afterTitle: await __twTab.title() ?? null, keypressReturned: true, inputMethod: "focused-keypress", key: ${quote(input.key)}, settleCompleted: false }; await __twTab.playwright.waitForTimeout(250); __twPayload.settleCompleted = true;`;
      return `${header}\nconst __twInfo = (await __twBrowser.user.openTabs()).find((tab) => tab.providerTabId === ${quote(input.providerTabId)}); if (!__twInfo) throw new Error("TOOLWIRE_BROWSER_TAB_STALE"); ${envelope("keypress", body)}`;
    }
    ...
    case OPERATIONS.download: {
      const source = locatorSource(input.target, input.binding);
      const body = `const __twBeforeUrl = (await __twTab.url()) ?? __twInfo.url ?? null; if (__twBeforeUrl !== (${quote(input.expectedUrl)})) throw new Error("TOOLWIRE_BROWSER_ACTION_URL_CHANGED"); ${source} if (__twCount !== 1) throw new Error("TOOLWIRE_BROWSER_LOCATOR_COUNT:" + __twCount); if (!(await __twLocator.isVisible())) throw new Error("TOOLWIRE_BROWSER_LOCATOR_NOT_VISIBLE"); if (!(await __twLocator.isEnabled())) throw new Error("TOOLWIRE_BROWSER_LOCATOR_NOT_ENABLED"); __twDispatchAttempted = true; const __twDownloadPromise = __twTab.playwright.waitForEvent("download", { timeoutMs: 10000 }); await __twLocator.click({ timeoutMs: 5000 }); const __twDownload = await __twDownloadPromise; const __twPath = typeof __twDownload?.path === "function" ? await __twDownload.path() : null; __twPayload = { beforeUrl: __twBeforeUrl, afterUrl: await __twTab.url() ?? null, afterTitle: await __twTab.title() ?? null, snapshot: await __twTab.playwright.domSnapshot(), clickReturned: true, downloadConfirmed: true, downloadPath: __twPath, pathError: null, readbackError: null };`;
      return `${header}\nconst __twInfo = (await __twBrowser.user.openTabs()).find((tab) => tab.providerTabId === ${quote(input.providerTabId)}); if (!__twInfo) throw new Error("TOOLWIRE_BROWSER_TAB_STALE"); ${envelope("download", body)}`;
    }
    case OPERATIONS.upload: {
      const source = locatorSource(input.target, input.binding);
      const body = `const __twBeforeUrl = (await __twTab.url()) ?? __twInfo.url ?? null; if (__twBeforeUrl !== (${quote(input.expectedUrl)})) throw new Error("TOOLWIRE_BROWSER_ACTION_URL_CHANGED"); ${source} if (__twCount !== 1) throw new Error("TOOLWIRE_BROWSER_LOCATOR_COUNT:" + __twCount); if (!(await __twLocator.isVisible())) throw new Error("TOOLWIRE_BROWSER_LOCATOR_NOT_VISIBLE"); if (!(await __twLocator.isEnabled())) throw new Error("TOOLWIRE_BROWSER_LOCATOR_NOT_ENABLED"); const __twChooserPromise = __twTab.playwright.waitForEvent("filechooser", { timeoutMs: 10000 }); __twDispatchAttempted = true; await __twLocator.click({ timeoutMs: 5000 }); const __twChooser = await __twChooserPromise; const __twMultiple = __twChooser.isMultiple(); await __twChooser.setFiles([${quote(input.filePath)}], { timeoutMs: 10000 }); __twPayload = { beforeUrl: __twBeforeUrl, afterUrl: await __twTab.url() ?? null, afterTitle: await __twTab.title() ?? null, snapshot: await __twTab.playwright.domSnapshot(), chooserConfirmed: true, multiple: __twMultiple, setFilesReturned: true, readbackError: null };`;
      return `${header}\nconst __twInfo = (await __twBrowser.user.openTabs()).find((tab) => tab.providerTabId === ${quote(input.providerTabId)}); if (!__twInfo) throw new Error("TOOLWIRE_BROWSER_TAB_STALE"); ${envelope("upload", body)}`;
    }
